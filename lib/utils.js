...
const getFailedScreenshot = (screenshotFileBaseName, retries) => {
  ...
  return new Promise(resolve => {
    ...
    console.log('screenshotFileBaseName: ', screenshotFileBaseName); // original screenshotFileBaseName
    console.log('testName: ', testName);                             // filename after replace
    filesAll = filesAll.concat(glob.sync(`**/${testName} (failed).png`));
    console.log('glob.sync(`**/${testName} (failed).png`): ', glob.sync(`**/${testName} (failed).png`)); // an empty array means no screenshot found
