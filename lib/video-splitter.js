...
const splitFullVideoIntoParts = async (suiteTestsDurationsMap, logger) => {
  ...
  for (let suiteInfo of suiteTestsDurationsMap.entries()) {
    ...
    console.log('fs.existsSync(fullVideoFilePath)', fs.existsSync(fullVideoFilePath));
    console.log('fs.readdir("/workdir/cypress/videos/")', fs.readdir('/workdir/cypress/videos/', (err, files) => console.log(files)));
