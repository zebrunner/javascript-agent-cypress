...
class ZebrunnerApiClient {
  ...
  constructor(reporterConfig) {
    ...
    this.httpClient = new HttpClient(reporterConfig.reporterOptions.reportingServerHostname)
  ...
  refreshToken = async() => {
    if(!this.accessToken) {
      const res = await this.httpClient.callPost(urls.URL_REFRESH, getRefreshToken(this.reporterConfig.reporterOptions.reportingServerAccessToken), jsonHeaders.headers)
  ...
  registerTestRunStart = async(suite) => {
    ...
    if(headers) {
      var project = (this.reporterConfig.reporterOptions.reportingProjectKey) ? this.reporterConfig.reporterOptions.reportingProjectKey : 'DEF';
      var testRunStartBody = getTestRunStart(suite, this.reporterConfig.reporterOptions)
