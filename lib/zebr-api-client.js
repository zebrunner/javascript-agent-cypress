...
class ZebrunnerApiClient {
  ...
  async startTest(test) {
    if (this.runId) {
      ...
      // fs.writeFile(__dirname + `/zbrTestsData${process.env.HOSTNAME || ''}.txt`, JSON.stringify({ testsData: this.storedTestsMap, runId: this.runId }, mapJsonReplacer), (err) => {
      //   if (err) {
  ...
  async startTestSession(test) {
    console.log('startTestSession');
    console.log('test', test);
    console.log('this.testsMap.get(test.uniqueId)', !!this.testsMap.get(test.uniqueId));
    if (this.testsMap.get(test.uniqueId)) {
        ...
        return this.getHeadersWithAuth(jsonHeaders).then((headers) => {
          return this.getCurrentBrowser().then(currentBrowser => {
            ...
            resolve(this.httpClient.callPost(url, testSession, headers).then(async (res) => {
              ...
              // this.storedSessionsId.set(test.uniqueId, res.data.id);
              this.setTestsData(`${this.runId}${process.env.HOSTNAME || ''}`, JSON.stringify(this.storedTestsMap, mapJsonReplacer))
                .then(() => {
                  this.setTestsData(`runId${process.env.HOSTNAME || ''}`, this.runId)
                    // .then(() => {
                    //   this.setTestsData(`storedSessionsId${process.env.HOSTNAME || ''}`, JSON.stringify(this.storedSessionsId, mapJsonReplacer))
                    // });
                })
  ...
  async setTestsData(key, value) {
    console.log('setTestsData value', value);
    const headers = await this.getHeadersWithAuth(jsonHeaders);
