...
var { getFilesizeInBytes, writeJsonToFile, mapJsonReplacer, executeShell } = require("./utils");
...
class ZebrunnerApiClient {
  ...
  async startTest(test) {
    if (this.runId) {
      ...
      this.setTestsData(`${this.runId}${process.env.HOSTNAME || ''}`, JSON.stringify(this.storedTestsMap, mapJsonReplacer))
      ...
      this.testsMap.get(test.uniqueId).promiseStartSession = new Promise(resolve => {
        return this.getHeadersWithAuth(jsonHeaders).then((headers) => {
          return this.getCurrentBrowser().then(currentBrowser => {
            ...
            this.sessionData = {...testSession};
            console.log('testSession.sessionId', testSession.sessionId);
            // const start_rec_command = `/opt/start-capture-artifacts.sh ${testSession.sessionId} >> /tmp/video.log 2>&1`;
            // (0, executeShell)(start_rec_command, '[ZEBRUNNER_CYPRESS_AGENT] start capturing artifacts');
            ...
            resolve(this.httpClient.callPost(url, testSession, headers).then(async (res) => {
              if (this.testsMap.get(test.uniqueId) && res?.data?.id) {
                ...
                this.setTestsData(`${this.runId}${process.env.HOSTNAME || ''}`, JSON.stringify(this.storedTestsMap, mapJsonReplacer))
                  .then(() => {
                    this.setTestsData(`runId${process.env.HOSTNAME || ''}`, this.runId)
                  })
              }
    ...
    if (this.testsMap.get(test.uniqueId)) {
      return this.getHeadersWithAuth(jsonHeaders).then(headers => {
        return this.testsMap.get(test.uniqueId).promiseStartSession.then(() => {
          ...
          // if (this.sessionData) {
          //   const stop_rec_command = `/opt/stop-capture-artifacts.sh ${this.sessionData.sessionId}`;
          //   (0, executeShell)(stop_rec_command, '[ZEBRUNNER_CYPRESS_AGENT] stop capturing artifacts');
  ...
  async setTestsData(key, value) {
    const headers = await this.getHeadersWithAuth(jsonHeaders);
    const url = `${urls.URL_ZE_TESTS_DATA_SAVE}/key:${key}`;

    return this.httpClient.callPut(url, { value }, headers);
  }

  async getStoredTestData(key) {
    const headers = await this.getHeadersWithAuth(jsonHeaders);
    const url = `${urls.URL_ZE_TESTS_DATA_SAVE}/key:${key}`;
    return this.httpClient.callGet(url, headers);
  }

  async getAllStoredTestsData() {
    const headers = await this.getHeadersWithAuth(jsonHeaders);
    return this.httpClient.callGet(urls.URL_ZE_TESTS_DATA_SAVE, headers);
  }
}
