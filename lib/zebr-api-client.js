...
class ZebrunnerApiClient {
  ...
  async getMultipardDataHeadersWithAuth() {
    var authToken = await this.refreshToken()
    if(authToken) {
  ...
  async getAllTestsFinishedPromise() {
    if(this.runId) {
      var promises = Array.from(this.testsMap.values()).map(i => i.promiseFinish);
      logToFile(`1 ${promises}`)
  ...
  async finishTest(test, status, reason) {
    if(this.testsMap.get(test.uniqueId)) {
      this.testsMap.get(test.uniqueId).promiseFinish = new Promise(resolve => {
        this.getJsonHeadersWithAuth().then(headers => {
  ...
  async startTestSession(test) {
    if(this.testsMap.get(test.uniqueId)) {
      ...
      this.testsMap.get(test.uniqueId).promiseStartSession = call
    }
  }
  ...
  async finishTestSession(test) {
    if(this.testsMap.get(test.uniqueId)) {
      return new Promise(resolve => {
        this.getJsonHeadersWithAuth().then(headers => {
          let startSessionPromise = this.testsMap.get(test.uniqueId).promiseStartSession
          if(!startSessionPromise) {
            var attempts = 5
            const waitForSessionStartPromise = async()  => {
              console.log("waiting for promiseStartSession has started");
              while(!this.testsMap.get(test.uniqueId).promiseStartSession && (attempts-- > 0))
                  await new Promise(resolve => setTimeout(resolve, 200));
          ...
          if(startSessionPromise) {
            startSessionPromise.then(() => {
              ...
              resolve(this.httpClient.callPut(url, testSession, headers))
            })
          }
        })
      })
  ...
  sendScreenshot(test, imgPath) {
    if(this.testsMap.get(test.uniqueId)) {
      ...
      var headers = this.getImageHeadersWithAuth();
  ...
  sendVideo(videoFilePath, runId, zbrSessionId) {
    try {
      if (fs.existsSync(videoFilePath)) {
        ...
        logToFile(`post video url: ${url}`)
        var headers = this.getMultipardDataHeadersWithAuth()
