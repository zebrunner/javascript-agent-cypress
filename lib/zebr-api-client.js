...
class ZebrunnerApiClient {
  ...
  async registerTestRunStart(suite, testRunUuid) {
    ...
    if (headers) {
      ...
      return this.httpClient
        .callPost(urls.URL_REGISTER_RUN.replace('${project}', this.getProjectKey()), testRunStartBody, headers)
        .then((res) => {
          this.runId = res?.data?.id;
          fs.writeFile(
            `${__dirname}/zbrTestsData${process.env.HOSTNAME || ''}.txt`,
            JSON.stringify({ runId: this.runId }),
            (err) => {
              if (err) {
  ...
  async startTest(test) {
    if (this.runId) {
      ...
      this.logger.info(
        ...
        `Test '${test.fullTitle}' was registered by id ${testStartResponse.data.id}`,
      );
      ...
      this.testsMap.get(test.uniqueId).promiseStartSession = new Promise((resolve) =>
        this.getHeadersWithAuth(jsonHeaders).then((headers) =>
          this.getCurrentBrowser().then((currentBrowser) => {
            ...
            resolve(
              this.httpClient.callPost(url, testSession, headers).then(async (res) => {
                ...
                this.logger.info(
                  path.basename(__filename),
                  `Session with id ${res?.data?.id} was registered for test '${test.fullTitle}'`,
                );
              }),
            );
          }),
    ...
    if (this.testsMap.get(test.uniqueId)) {
      return this.getHeadersWithAuth(jsonHeaders).then((headers) =>
        this.testsMap.get(test.uniqueId).promiseStartSession.then(() => {
          ...
          const url = urls.URL_UPDATE_SESSION.replace('${testRunId}', this.runId).replace(
            '${testSessionId}',
            this.testsMap.get(test.uniqueId).zbrSessionId,
          );
  ...
  async revertTestRegistration(test) {
    if (this.runId) {
      const url = urls.URL_REVERT_TEST_REGISTRATION.replace('${testRunId}', this.runId).replace(
        '${testId}',
        this.testsMap.get(test.uniqueId)?.zbrTestId,
      );
      const headers = await this.getHeadersWithAuth(jsonHeaders);

      await this.httpClient.callDelete(url, headers);
    }
  }

  async sendLaunchArtifactReference(requestBody) {
    if (this.runId) {
      const url = urls.URL_ATTACH_LAUNCH_ARTIFACT_REFERENCE.replace('${testRunId}', this.runId);
      const headers = await this.getHeadersWithAuth(jsonHeaders);

      if (requestBody.items.length !== 0) {
        await this.httpClient.callPut(url, requestBody, headers);
      }
    }
  }

  async sendTestArtifactReference(test, requestBody) {
    if (this.runId) {
      const url = urls.URL_ATTACH_TEST_ARTIFACT_REFERENCE.replace('${testRunId}', this.runId).replace(
        '${testId}',
        this.testsMap.get(test.uniqueId).zbrTestId,
      );
      const headers = await this.getHeadersWithAuth(jsonHeaders);
  ...
  async uploadLaunchArtifact(contentTypeHeader, file) {
    if (this.runId) {
      const url = urls.URL_ATTACH_LAUNCH_ARTIFACT.replace('${testRunId}', this.runId);
      const headers = await this.getHeadersWithAuth({ headers: { 'Content-Type': contentTypeHeader, Accept: '*/*' } });

      await this.httpClient.callPost(url, file, headers);
    }
  }

  async uploadTestArtifact(contentTypeHeader, file, test) {
    const url = urls.URL_ATTACH_TEST_ARTIFACT.replace('${testRunId}', this.runId).replace(
      '${testId}',
      this.testsMap.get(test.uniqueId).zbrTestId,
    );
    const headers = await this.getHeadersWithAuth({ headers: { 'Content-Type': contentTypeHeader, Accept: '*/*' } });
  ...
  sendScreenshot(test, imgPath) {
    if (this.testsMap.get(test.uniqueId)) {
      ...
      const url = urls.URL_SEND_SCREENSHOT.replace('${testRunId}', this.runId).replace(
        '${testId}',
        this.testsMap.get(test.uniqueId).zbrTestId,
      );
  ...
  async sendLaunchLabels(requestBody) {
    // same as sendRunLabels, but receive custom labels from custom cy method attachZbrLaunchLabel
    if (this.runId) {
      const url = urls.URL_ATTACH_TEST_RUN_LABELS.replace('${testRunId}', this.runId);
      const headers = await this.getHeadersWithAuth(jsonHeaders);
  ...
  async sendTestLabels(test, requestBody) {
    if (this.runId) {
      const url = urls.URL_ATTACH_TEST_LABELS.replace('${testRunId}', this.runId).replace(
        '${testId}',
        this.testsMap.get(test.uniqueId)?.zbrTestId,
      );
      const headers = await this.getHeadersWithAuth(jsonHeaders);
  ...
  addTestCases(test, testCase) {
    if (this.testsMap.get(test.uniqueId)) {
      ...
      if (zbrTestId) {
        ...
        this.testsMap.get(test.uniqueId).testCases = this.testsMap
          .get(test.uniqueId)
          .testCases.filter(
            (existingTestCase) =>
  ...
  getDefaultTestCaseStatus(testStatus) {
    ...
    if (testStatus === testStatuses.SKIPPED) {
      return this.configResolver.getReportingTcmTestCaseStatusOnSkip();
    }
  }
