...
var { getFilesizeInBytes, writeJsonToFile } = require("./utils");
...
class ZebrunnerApiClient {
  ...
  async parseResultsAndSendVideo(suiteTestsDurationsMap) {
    var promises = [];
    console.log('----- parseResultsAndSendVideo -----');
    console.log('suiteTestsDurationsMap: ', suiteTestsDurationsMap);
    if (suiteTestsDurationsMap) {
      ...
      suiteTestsDurationsMap.forEach(tests => {
        tests.forEach(test => {
          if (test.videoFilePath) {
            ...
            console.log(`video path: ${test.videoFilePath}`);
            ...
            console.log('----- video will not be pushed. video not exists -----');
            console.log('test.id: ', test?.id);
            console.log(`unexisted video path: ${test.videoFilePath}`);
          }
        })
      })
      ...
      console.log(`cant split and send video. there are no tests in suite`);
    } else {
      this.logger.info(path.basename(__filename), `cant split and send video. video is disabled`);
      console.log(`cant split and send video. video is disabled`);
    }

    if (promises.length > 0) {
      return Promise.all(promises)
        .then(() => {
          console.log('----- videos were sended -----');
        })
        .catch((err) => {
          console.log('----- err while sending video -----');
          console.log('err: ', err);
        });
    } else {
      console.log('----- empty promise for video resolved -----');
      return new Promise(resolve => { resolve() })
    }
  }
