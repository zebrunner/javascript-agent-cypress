...
class ZebrunnerApiClient {
  ...
  async sendLaunchLabels(requestBody) { // same as sendRunLabels, but receive custom labels from custom cy method attachZbrLaunchLabel
    if (this.runId) {
      const url = urls.URL_SET_RUN_LABELS.replace('${testRunId}', this.runId);
      console.log('sendLaunchLabels url', url);
      const headers = await this.getHeadersWithAuth(jsonHeaders);
  ...
  async sendTestLabels(test, requestBody) {
    if (this.runId) {
      const url = urls.URL_SET_TEST_LABELS.replace('${testRunId}', this.runId).replace('${testId}', this.testsMap.get(test.uniqueId)?.zbrTestId);
      console.log('sendTestLabels url', url);
      const headers = await this.getHeadersWithAuth(jsonHeaders);
