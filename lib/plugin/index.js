...
const registerZbrPlugin = (on, config) => {
  ...
  on('after:run', async (results) => {
    ...
    spinnies.succeed('testsDurationData', { text: `testsDurationData: ${testsDurationData}`});
    spinnies.succeed('testsData', { text: `testsData: ${testsData}` });
    spinnies.succeed('runId', { text: `runId: ${runId}`});
    ...
    if (testsDurationData) {
      ...
      spinnies.succeed('testsDurationData', { text: `Video splitter results: ${testsDurationData}`});
      testsDurationData.forEach(tests => {
        tests.forEach(test => {
          spinnies.succeed('promises', { text: `Video promises constructor. Condition: ${test.videoFilePath}`});
