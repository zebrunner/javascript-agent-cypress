  ...
  //   });
      ...
      testsDurationData?.forEach(tests => {
        tests.forEach(async (test) => {
          ...
          if (test.videoFilePath && testsData?.get(test.uniqueId)) {
            testStatusPromises.push(test.state === 'pending' ? deletePendingTest({...test, ...testsData.get(test.uniqueId)}) : setTestStatus({...test, ...testsData.get(test.uniqueId)}));
    ...
    const setTesteStatus = () => {
      return new Promise((resolve, reject) => {
        return Promise.all(testStatusPromises).then(() => resolve());
      })
    }
