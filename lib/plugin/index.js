...
const registerZbrPlugin = (on, config) => {
  ...
  //   if (afterRunEventInProgress) { return; }
      ...
      testsDurationData?.forEach(tests => {
        tests.forEach(async (test) => {
          ...
          if (test.videoFilePath && testsData?.get(test.uniqueId)) {
            ...
            promises.push({id: testsData.get(test.uniqueId).zbrTestId, value: sendVideo(test.videoFilePath, runId, testsData.get(test.uniqueId).zbrSessionId)});
          } else {
    ...
    if (videoArtifactPromises.length > 0) {
      return new Promise((resolve, reject) => {
        return Promise.all(videoArtifactPromises).then(() => {
