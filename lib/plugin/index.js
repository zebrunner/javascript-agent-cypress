...
const registerZbrPlugin = (on, config) => {
  ...
  //   });
        ...
        tests.forEach(async (test) => {
          ...
          if (test.videoFilePath && testsData?.get(test.uniqueId)) {
            if (test.state === 'pending') {
              ...
              testStatusPromises.push(setTestStatus({...test, ...testsData.get(test.uniqueId)}));
            }
    ...
    const setTesteStatus = () => {
      return new Promise((resolve, reject) => {
        return Promise.all(testStatusPromises).then((results) => {
          console.log('setTesteStatus results', results);
          return resolve();
        });
      })
    }
    ...
    const deleteTests = () => {
      return new Promise((resolve, reject) => {
        return Promise.all(testsToDelete).then((results) => {
          console.log('deleteTests results', results);
          return resolve();
        });
      })
    }
    ...
    // if (testsToDelete.length) {
      console.log('testsToDelete.length', testsToDelete.length);
      await deleteTests();
    // }

    // if (testStatusPromises.length) {
      console.log('testStatusPromises.length', testStatusPromises.length);
      await setTesteStatus();
    // }
