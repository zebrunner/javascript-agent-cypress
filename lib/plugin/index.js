...
const registerZbrPlugin = (on, config) => {
  ...
  on('after:run', async (results) => {
    ...
    console.log('index zbrTestsDurationData dirname', path.join(__dirname, '..', `zbrTestsDurationData${process.env.HOSTNAME || ''}.txt`));
    console.log('index zbrTestsData dirname', path.join(__dirname, '..', `zbrTestsData${process.env.HOSTNAME || ''}.txt`));

    const durationJsonData = await fs.readFileSync(path.join(__dirname, '..', `zbrTestsDurationData${process.env.HOSTNAME || ''}.txt`), { encoding:'utf8', flag:'r' } );
    console.log('durationJsonData', durationJsonData);
    const testsDurationData = await safelyParseJSON(durationJsonData) || {};
    const testsJsonData = await fs.readFileSync(path.join(__dirname, '..', `zbrTestsData${process.env.HOSTNAME || ''}.txt`), { encoding:'utf8', flag:'r' } );
    console.log('testsJsonData', testsJsonData);
    const { testsData, runId } = await safelyParseJSON(testsJsonData) || {};
