...
const registerZbrPlugin = (on, config) => {
  ...
  on('after:run', async (results) => {
    ...
    const durationJsonData = await fs.readFileSync(`../zbrTestsDurationData${process.env.HOSTNAME || ''}.txt`, { encoding:'utf8', flag:'r' } );
    console.log('durationJsonData', durationJsonData);
    const testsDurationData = await safelyParseJSON(durationJsonData) || {};
    const testsJsonData = await fs.readFileSync(`../zbrTestsData${process.env.HOSTNAME || ''}.txt`, { encoding:'utf8', flag:'r' } );
    ...
    let files = [`../zbrTestsDurationData${process.env.HOSTNAME || ''}.txt`, `../zbrTestsData${process.env.HOSTNAME || ''}.txt`];
