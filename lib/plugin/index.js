...
const registerZbrPlugin = (on, config) => {
  ...
  on('task', {
    ...
    zbr_attachZbrTestLabel({ labels }) {
      ...
      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.ATTACH_TEST_LABELS, {
          key,
          values,
        });
      });
    ...
    zbr_attachZbrLaunchLabel({ labels }) {
      ...
      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.ATTACH_LAUNCH_LABELS, {
          key,
          values,
        });
      });
    ...
    zbr_revertTestRegistration() {
      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.REVERT_TEST_REGISTRATION);
      });
      return null;
    },

    zbr_attachZbrTestArtifactReference({ artifactReference }) {
      const { name, value } = artifactReference;

      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.ATTACH_TEST_ARTIFACT_REFERENCE, {
          name,
          value,
        });
      });
    ...
    zbr_attachZbrLaunchArtifactReference({ artifactReference }) {
      const { name, value } = artifactReference;

      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.ATTACH_LAUNCH_ARTIFACT_REFERENCE, { name, value });
      });
      return null;
    },

    zbr_attachZbrTestArtifact({ artifact }) {
      const { pathOrBuffer, name } = artifact;
      const timestamp = new Date().getTime();

      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.ATTACH_TEST_ARTIFACT, {
          pathOrBuffer,
          name,
          timestamp,
        });
    ...
    zbr_attachZbrLaunchArtifact({ artifact }) {
      const { pathOrBuffer, name } = artifact;
      const timestamp = new Date().getTime();

      connectPromise.then(() => {
        ipc.of[ipcConnectionAlias].emit(EVENTS.ATTACH_LAUNCH_ARTIFACT, {
          pathOrBuffer,
          name,
          timestamp,
        });
  ...
  on('after:run', async (results) => {
    ...
    return new Promise((resolve) => setTimeout(resolve, process.env.ZBR_RUN_END_TIMEOUT || 1000)).then(async () => {
      ...
      const refreshToken = async () => {
        if (!accessToken) {
          const res = await httpClient.callPost(
            urls.URL_REFRESH,
            getRefreshToken(configResolver.getReportingServerAccessToken()),
            jsonHeaders.headers,
            true,
          );
      ...
      if (testRunLabel) {
        ...
        await httpClient.callPut(
          urls.URL_ATTACH_TEST_RUN_LABELS.replace('${testRunId}', runId),
          { items: [{ key: 'tags', value: testRunLabel }] },
          headers,
        );
      }
      ...
      const registerTestRunFinish = async () => {
        ...
        return httpClient.callPut(
          ...
          {
            endedAt: new Date(Date.now() - (process.env.ZBR_RUN_END_TIMEOUT || 1000)),
          },
          headers,
        );
      };
