...
const { parseSuiteStartObject, parseSuiteEndObject, parseTestInfo, parseSuiteTestsDurations, testParse } = require('./object-transformer');
...
const { getTotalSpecs } = require('./utils');
...
class ZbrReporter extends Mocha.reporters.Base {
  constructor(runner, config) {
    ...
    this.runner
      ...
      .once(EVENT_RUN_END, () => {
        ZbrReporter.totalLaunches = getTotalSpecs(cypressConfig);
        if (ZbrReporter.currentLaunch === ZbrReporter.totalLaunches) {
          this.worker.send({
            event: EVENT_RUN_END
          });
        }
...
ZbrReporter.currentLaunch = 0;
ZbrReporter.totalLaunches = 0;
