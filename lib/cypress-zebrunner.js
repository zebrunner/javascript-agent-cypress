  ...
  constructor(runner, config) {
    ...
    this.runner
      .once(EVENT_RUN_BEGIN, () => {
        console.log('----- Run begin event fired -----');
        this.worker.send({
      ...
      .on(EVENT_SUITE_BEGIN, (suite) => {
        console.log('----- Suite begin event fired -----');
        this.worker.send({
      ...
      .on(EVENT_SUITE_END, (suite) => {
        console.log('----- Suite end event fired -----');
        if (suite.title === '') {
      ...
      .on(EVENT_TEST_BEGIN, test => {
        console.log('----- Test begin event fired -----');
      ...
      .on(EVENT_TEST_PASS, test => {
        console.log('----- Test passed event fired -----');
      ...
      .on(EVENT_TEST_FAIL, (test, err) => {
        console.log('----- Test fail event fired -----');
        console.log(`----- Test id: ${test.id} -----`);
        console.log(`----- Err: ${err} -----`);
        getCypressConfig().then(() => {
          this.worker.send({
      ...
      .once(EVENT_RUN_END, async () => {
        console.log('----- Run end event fired -----');
        sleep(process.env.ZBR_VIDEO_PROCESSING_TIMEOUT_MS || 15000);
        this.worker.send({
          event: EVENT_RUN_END
        });
      });
