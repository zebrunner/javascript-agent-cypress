...
process.on('message', async (message) => {
  ...
  switch (event) {
    ...
    case EVENT_RUN_BEGIN: {
      ...
      const start_vnc_command = '/opt/start-x11vnc.sh';
      (0, executeShell)(start_vnc_command, '[ZEBRUNNER_CYPRESS_AGENT] start vnc session');
      console.log('ZEBRUNNER REPORTER STARTED');
      if (fs.existsSync(path.join(__dirname, '../../..', 'cypress-cucumber-preprocessor'))) {
        console.log('cypress-cucumber-preprocessor exist');
        const { shouldProceedCurrentStep, getEnvTags } = require('../../../cypress-cucumber-preprocessor/lib/tagsHelper');
        console.log('shouldProceedCurrentStep', shouldProceedCurrentStep);
        console.log('getEnvTags', getEnvTags);
      } else {
        console.log('cypress-cucumber-preprocessor not exist');
      }
      break;
    }
          ...
          .then(() => {
            ...
            zebrunnerApiClient.startTestSession(message.test, sessionId)
              .then(() => {
                ...
                const start_rec_command = `/opt/start-capture-artifacts.sh ${sessionId} >> /tmp/video.log 2>&1`;
    ...
    case EVENT_TEST_END: {
      ...
      if (sessionId) {
        ...
        const stop_rec_command = `/opt/stop-capture-artifacts.sh ${sessionId}`;
      ...
      if (message.test.status.toLowerCase() === 'pending') {
        ...
        if (sessionId) {
          const upload_vid_command = `/opt/upload-artifacts.sh ${sessionId}`;
          (0, executeShell)(upload_vid_command, '[ZEBRUNNER_CYPRESS_AGENT] video upload');
        }
