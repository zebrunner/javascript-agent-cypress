  ...
  switch (event) {
    ...
    case EVENT_SUITE_BEGIN: {
      ...
      if (message.suite.title) {
        console.log('EVENT_SUITE_BEGIN', message.suite);
          ...
          contextExchangesPromise.then(() => {
            return runStartPromise.then(() => {
              ...
              return zebrunnerApiClient.startTest(message.test).then(() => {
                ...
                return Promise.all([
                  await zebrunnerApiClient.sendLogs(message.test, 'INFO', testLogs),
                  await zebrunnerApiClient.startTestSession(message.test),
                  await zebrunnerApiClient.sendRunLabels(),
                ])
              })
            })
          }))
    ...
    case EVENT_RUN_END: {
      ...
      console.log('zbrTestsDurationData directory', __dirname + `/zbrTestsDurationData${process.env.HOSTNAME || ''}.txt`);
      fs.writeFile(__dirname + `/zbrTestsDurationData${process.env.HOSTNAME || ''}.txt`, JSON.stringify(suiteTestsDurationsMap, mapJsonReplacer), (err) => {
        if (err) {
