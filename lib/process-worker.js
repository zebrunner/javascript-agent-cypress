...
process.on('message', async (message) => {
  ...
  switch (event) {
    ...
    case EVENT_TEST_FAIL: {
      finishTestPromises.push(startTestPromisesMap.get(message.test.uniqueId).then(() => {
        const promises = []
        console.log('----- EVENT_TEST_FAIL event -----');
        console.log('test: ', message.test);
        console.log('test id: ', message.test?.id);
        ...
        promises.push(getFailedScreenshot(message.test.screenshotFileBaseName, message.test.retries).then(screenshots => {
          console.log('----- EVENT_TEST_FAIL screenshots -----');
          ...
          if (screenshots.length > 0) {
            ...
            return Promise.all(promises)
              .then(() => {
                console.log('----- screenshots promises were resolved -----');
              }).catch((err) => {
                console.log('----- screenshots promises err -----');
                console.log('err: ', err);
              })
          } else {
          ...
          console.log('----- EVENT_TEST_FAIL screenshots failed -----');
          console.log('err: ', err);
        }))
        promises.push(zebrunnerApiClient.finishTestSession(message.test));
        promises.push(zebrunnerApiClient.finishTest(message.test, 'FAILED', message.err.message));
        console.log('----- overall failed test data to send -----');
        ...
        return Promise.all(promises).then(() => {
          ...
          console.log(`----- Failed to send 'failed' test data -----`);
          console.log('err: ', err);
        });
      }))
      break;
    ...
    case EVENT_RUN_END: {
      console.log('----- EVENT_RUN_END event -----');
      this.logger.info(path.basename(__filename), 'ZEBRUNNER REPORTER FINISHED')
      const promises = [];
      console.log('finishTestPromises length: ', finishTestPromises.length);
      console.log('finishTestPromises: ', finishTestPromises);
      // additional logic for parallelized runs
      promises.push(Promise.all(finishTestPromises).then(() => {
        console.log('----- all finish test promises are successfsuly done -----');
        ...
        console.log('----- finish test promises are done with error -----');
        console.log('err: ', err);
      ...
      Promise.all(promises)
        .then(() => {
          return Promise.all(zebrunnerApiClient.parseResultsAndSendVideo(suiteTestsDurationsMap))
            .then(() => {
              console.log('----- zebrunnerApiClient parseResultsAndSendVideo success -----');
            })
            .catch((err) => {
              console.log('----- zebrunnerApiClient parseResultsAndSendVideo error -----');
              console.log('err: ', err);
            });
        })
        .then(async () => {
