...
process.on('message', async (message) => {
  ...
  switch (event) {
          ...
          contextExchangesPromise.then(() => {
            return runStartPromise.then(() => {
              ...
              return zebrunnerApiClient.startTest(message.test).then(() => {
                zebrunnerApiClient.startTestSession(message.test).then(() => {

                  // TODO: start video recording (session id (check res?))
                  const start_rec_command = `/opt/start-capture-artifacts.sh ${message.test.uniqueId} >> /tmp/video.log 2>&1`;
    ...
    case EVENT_TEST_END: {
      ...
      console.log('message.test', message.test);
      // TODO: stop video recording, kill ffmpeg with signal 15

      const stop_rec_command = `/opt/stop-capture-artifacts.sh ${message.test.uniqueId}`;
