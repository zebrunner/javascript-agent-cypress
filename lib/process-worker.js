...
const { getFailedScreenshot, sleep, mapJsonReplacer, executeShell } = require('./utils');
...
process.on('message', async (message) => {
  ...
  switch (event) {
    ...
    case EVENT_TEST_BEGIN: {
      ...
      if (process.env.REPORTING_RUN_CONTEXT) {
        startTestPromisesMap.set(message.test.uniqueId, // TODO: remove code duplicate
          contextExchangesPromise.then(() => {
            return runStartPromise.then(() => {
              ...
              return zebrunnerApiClient.startTest(message.test).then(() => {
                sessionId = uuidv4();
                zebrunnerApiClient.startTestSession(message.test, sessionId).then(() => {
        ...
        if (startTestPromisesMap.get(message.test.uniqueId)) {
          ...
          finishTestPromises.push(startTestPromisesMap.get(message.test.uniqueId).then(() => {
            ...
            if (message.test.status.toLowerCase() === 'failed') {
              promises.push(zebrunnerApiClient.sendLogs(message.test, 'ERROR', [`Failure message: ${message.test?.err}`]))
