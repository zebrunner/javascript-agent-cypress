...
process.on('message', async (message) => {
  ...
  switch (event) {
          ...
          contextExchangesPromise.then(() => {
            return runStartPromise.then(() => {
              ...
              return zebrunnerApiClient.startTest(message.test).then(() => {
                zebrunnerApiClient.startTestSession(message.test).then((sessionId) => {
                  console.log('sessionId', sessionId);
                  testSessionId = sessionId;

                  // TODO: start video recording (session id (check res?))
                  const start_rec_command = `/opt/start-capture-artifacts.sh ${testSessionId} >> /tmp/video.log 2>&1`;
    ...
    case EVENT_TEST_END: {
      ...
      if (testSessionId) {
        const stop_rec_command = `/opt/stop-capture-artifacts.sh ${testSessionId}`;
