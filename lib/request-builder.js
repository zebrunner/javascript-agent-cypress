...
const getTestRunStart = (suite, reporterConfig, testRunUuid = null) => {
  ...
  const testRunStartBody = {
    ...
    config: {},
    notifications: {
      notifyOnEachFailure: configResolver.getNotifyOnEachFailure(),
      summarySendingPolicy: configResolver.getSummarySendingPolicy(),
      targets: [],
    }
  };
  ...
  if (configResolver.getSlackChannels()) {
    testRunStartBody.notifications.targets.push({ type: 'SLACK_CHANNELS', value: configResolver.getSlackChannels() });
  }
  if (configResolver.getEmailRecipients()) {
    testRunStartBody.notifications.targets.push({ type: 'EMAIL_RECIPIENTS', value: configResolver.getEmailRecipients() });
  }
