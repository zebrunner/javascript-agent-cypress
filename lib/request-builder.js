...
const urls = {
  ...
  URL_ATTACH_LAUNCH_ARTIFACT_REFERENCE: '/api/reporting/v1/test-runs/${testRunId}/artifact-references',
  URL_ATTACH_TEST_ARTIFACT_REFERENCE: '/api/reporting/v1/test-runs/${testRunId}/tests/${testId}/artifact-references',

  URL_ATTACH_LAUNCH_ARTIFACT: '/api/reporting/v1/test-runs/${testRunId}/artifacts',
  URL_ATTACH_TEST_ARTIFACT: '/api/reporting/v1/test-runs/${testRunId}/tests/${testId}/artifacts',
...
const getTestRunStart = (suite, reporterConfig, testRunUuid = null) => {
  ...
  if (configResolver.getReportingRunTreatSkipsAsFailures()) {
    testRunStartBody.config.treatSkipsAsFailures = configResolver.getReportingRunTreatSkipsAsFailures();
  }
...
const getTestRunLabels = (reporterOptions) => {
  ...
  if (reporterOptions.reportingLaunchLocale) {
    testRunLabelsBody.items.push({ key: 'com.zebrunner.app/sut.locale', value: reporterOptions.reportingLaunchLocale });
  }
  if (reporterOptions.reportingTestrailEnabled) {
    if (reporterOptions.reportingTestrailEnabled) {
      testRunLabelsBody.items.push({
        key: testrailLabels.L_SYNC_ENABLED,
        value: reporterOptions.reportingTestrailEnabled,
      });
    }
    ...
    if (reporterOptions.reportingTestrailTestrunName) {
      testRunLabelsBody.items.push({
        key: testrailLabels.L_RUN_NAME,
        value: reporterOptions.reportingTestrailTestrunName,
      });
    }
    if (reporterOptions.reportingTestrailMilestone) {
      testRunLabelsBody.items.push({
        key: testrailLabels.L_MILESTONE,
        value: reporterOptions.reportingTestrailMilestone,
      });
    }
    if (reporterOptions.reportingTestrailAssignee) {
      testRunLabelsBody.items.push({
        key: testrailLabels.L_ASSIGNEE,
        value: reporterOptions.reportingTestrailAssignee,
      });
    }
    if (reporterOptions.reportingTestrailIncludeAll) {
      testRunLabelsBody.items.push({
        key: testrailLabels.L_INCLUDE_ALL,
        value: reporterOptions.reportingTestrailIncludeAll,
      });
    }
  }
  if (reporterOptions.reportingXrayEnabled) {
    ...
    if (reporterOptions.reportingXrayTestExecutionKey) {
      testRunLabelsBody.items.push({
        key: xrayLabels.L_EXECUTION_KEY,
        value: reporterOptions.reportingXrayTestExecutionKey,
      });
    }
  }
